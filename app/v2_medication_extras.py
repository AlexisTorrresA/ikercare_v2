...
def _sos_meta(db: Session, medication_id: int) -> CareRecordMeta | None:
    return db.scalar(select(CareRecordMeta).where(CareRecordMeta.entity_type == "medication", CareRecordMeta.entity_id == medication_id, CareRecordMeta.key == "is_sos"))
...
def _is_sos(db: Session, med: CareMedication) -> bool:
    ...
    if row:
        return row.value == "1"
    text = (med.frequency or "").casefold()
    return "sos" in text or "rescate" in text


def _serialize_sos(med: CareMedication) -> dict:
    return {
        "id": med.id,
        "name": med.name,
        "dose": med.dose,
...
@medication_extra_api.get("/patients/{patient_id}/medications-sos")
def list_sos_medications(patient_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)) -> list[dict]:
    _membership(db, user.id, patient_id)
    meds = db.scalars(select(CareMedication).where(CareMedication.patient_id == patient_id, CareMedication.active.is_(True)).order_by(CareMedication.name)).all()
    return [_serialize_sos(med) for med in meds if _is_sos(db, med)]


@medication_extra_api.get("/patients/{patient_id}/medications/{medication_id}/sos")
def get_medication_sos(patient_id: int, medication_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)) -> dict:
    _membership(db, user.id, patient_id)
...
def update_medication_sos(patient_id: int, medication_id: int, payload: dict = Body(...), db: Session = Depends(get_db), user: User = Depends(get_current_user), _: None = Depends(verify_csrf)) -> dict:
    _require_role(db, user.id, patient_id, {"owner", "editor"})
    med = _medication(db, patient_id, medication_id)
    is_sos = bool(payload.get("is_sos"))
...
@medication_extra_api.post("/patients/{patient_id}/medications/{medication_id}/sos-use", status_code=201)
def register_sos_use(patient_id: int, medication_id: int, payload: dict = Body(default={}), db: Session = Depends(get_db), user: User = Depends(get_current_user), _: None = Depends(verify_csrf)) -> dict:
    _require_role(db, user.id, patient_id, {"owner", "editor"})
    med = _medication(db, patient_id, medication_id)
    ...
    event = CareMedicationEvent(medication_id=medication_id, occurred_at=occurred_at, notes=notes, created_by_user_id=user.id)
    db.add(event)
    db.flush()
...
def permanently_delete_medication(patient_id: int, medication_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user), _: None = Depends(verify_csrf)) -> dict:
    ...
    med = _medication(db, patient_id, medication_id)
    name, source = med.name, med.source
...
def delete_treatment_history_item(patient_id: int, medication_id: int, history_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user), _: None = Depends(verify_csrf)) -> dict:
    _require_role(db, user.id, patient_id, {"owner", "editor"})
    _medication(db, patient_id, medication_id)
    row = db.scalar(select(MedicationTreatmentHistory).where(MedicationTreatmentHistory.id == history_id, MedicationTreatmentHistory.medication_id == medication_id))
    if not row:
