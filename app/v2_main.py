...
# Timeline sin duplicados debe tener prioridad sobre la ruta histórica anterior.
...
async def friendly_registration_validation(request: Request, exc: RequestValidationError):
    labels = {
        "username": "usuario", "email": "correo", "display_name": "nombre", "password": "contraseña",
        "accept_privacy": "aceptación de privacidad", "guardian_attestation": "declaración de autorización",
    }
    messages: list[str] = []
...
def v2_home(request: Request, db: DbDep):
    ...
    return templates.TemplateResponse(
        request=request,
        name="v2.html",
        context={"csrf_token": request.session.get("csrf_token", ""), "username": user.username, "privacy_version": PRIVACY_VERSION},
    )
