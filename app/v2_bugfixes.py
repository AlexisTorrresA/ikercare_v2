...
def get_elimination_record(patient_id: int, item_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)) -> dict:
    ...
    return {
        ...
        "diaper_status": item.diaper_status,
        "urine_amount": item.urine_amount,
        "urine_color": item.urine_color,
        "stool_description": item.stool_description,
        "notes": item.notes,
    }


@bugfix_api.put("/patients/{patient_id}/elimination/{item_id}")
def update_elimination_record(
    patient_id: int,
    item_id: int,
