...
logger = logging.getLogger("ikercare.clinical_alias")
clinical_alias_api = APIRouter(prefix="/api/v2", tags=["IkerCare hospitalization report aliases"])


@clinical_alias_api.get("/patients/{patient_id}/hospitalizations/{hospitalization_id}/hospital-report")
...
@clinical_alias_api.get("/patients/{patient_id}/hospitalizations/{hospitalization_id}/hospital-report.pdf")
def hospital_report_pdf(patient_id: int, hospitalization_id: int, use_ai: bool = True, db: Session = Depends(get_db), user: User = Depends(get_current_user)) -> Response:
    ...
    try:
        ...
        return Response(data, media_type="application/pdf", headers={"Content-Disposition": f'attachment; filename="IkerCare-hospitalizacion-{hospitalization_id}.pdf"', "Content-Length": str(len(data)), "Cache-Control": "private, no-store", "X-Content-Type-Options": "nosniff"})
    except HTTPException:
        raise
    except Exception as exc:
        logger.exception("Hospital PDF failed patient=%s hospitalization=%s", patient_id, hospitalization_id)
        raise HTTPException(status_code=500, detail="No fue posible generar el PDF. Inténtalo nuevamente en unos segundos.") from exc
